...
@section('contents')
    <div class="align-items-left align-items-md-center pt-3 pb-4">
		<div>
			<h2 class="pb-2 fw-bold"><i class="fas fa-dollar-sign"></i>&nbsp;&nbsp; Liste des paiements de scolarités</h2>
		</div>
        <hr>
	</div>
			...
			<tbody class="text-center">
                @if (count($scolarites)>0)
                    @foreach ($scolarites as $scolarite)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{nombre_format($scolarite->montant_paye)}} AR</td>
                            <td>{{date_formate($scolarite->date_paie,'d M Y')}}</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="6">Vous n'avez pas encore payé des scolarités</td>
                    </tr>
                @endif
