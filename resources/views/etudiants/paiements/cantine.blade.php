...
@section('contents')
    <div class="align-items-left align-items-md-center pt-3 pb-4">
		<div>
			<h2 class="pb-2 fw-bold"><i class="fas fa-dollar-sign"></i>&nbsp;&nbsp; Liste des cantines payées</h2>
		</div>
        <hr>
	</div>
			...
			<tbody class="text-center">
                @if (count($cantines)>0)
                    @foreach ($cantines as $cantine)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{ucfirst(date_formate($cantine->date_cantine,'l'))}}</td>
                            <td>{{date_formate($cantine->date_cantine,'d M Y')}}</td>
                            <td>{{nombre_format($cantine->montant)}} AR</td>
                            <td>{{date_formate($cantine->date_paye,'d M Y')}}</td>
                        </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="6">Vous n'avez pas encore payé des cantines</td>
                    </tr>
                @endif
