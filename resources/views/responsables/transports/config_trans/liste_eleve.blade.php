<div class="col-md-12 mx-1 my-3 pt-1">
    <div class="border border-dark w-100 p-3">
        <div class="row justify-content-between px-2 mx-3">
            ...
            @if ($eleve!="")
                <button class="btn btn-success btn-sm payer_transport" id="{{$eleve->id}}"><i class="fas fa-hand-holding-usd"></i>&nbsp;&nbsp;Payer</button>
            @endif
        </div>
        <hr>
                ...
                <tbody class="text-center" id="liste_paie">
                    @if (count($paiements)>0)
                        @foreach ($paiements as $paie)
                            <tr>
                                <td>{{$i++}}</td>
                                <td>{{date_formate($paie->mois,"F Y")}}</td>
                                <td>{{nombre_format($paie->montant)}} AR</td>
                                <td>{{date_formate($paie->date_payement,"d M Y")}}</td>
                            </tr>
                        @endforeach
                    @else
                        <tr>
